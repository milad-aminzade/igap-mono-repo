...
@NoArgsConstructor
public abstract class AbstractGreaterThanOrEqualJoinFilter<T extends Comparable<T>> extends FilterDecorator {
    private T value;
    private Filter filter;
    private List<String> sortedJoinColumns;

    public AbstractGreaterThanOrEqualJoinFilter(List<String> sortedJoinColumns, T value, Filter filter) {
    ...
    public List<Predicate> filters(CriteriaBuilder criteriaBuilder, Root root) {
        ...
        if (value != null) {
            ...
            filters.add(criteriaBuilder.greaterThanOrEqualTo(pathFromHierarchyJoin, value));
        }
        return filters;
    }
}
