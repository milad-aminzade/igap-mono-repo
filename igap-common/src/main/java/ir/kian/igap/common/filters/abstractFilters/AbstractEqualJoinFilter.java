...
@NoArgsConstructor
public abstract class AbstractEqualJoinFilter<T extends Comparable<T>> extends FilterDecorator {
    private T value;
    private Filter filter;
    private List<String> sortedJoinColumns;

    public AbstractEqualJoinFilter(List<String> sortedJoinColumns, T value, Filter filter) {
    ...
    public List<Predicate> filters(CriteriaBuilder criteriaBuilder, Root root) {
        ...
        if (value != null) {
            ...
            filters.add(criteriaBuilder.equal(pathFromHierarchyJoin, value));
        }
        return filters;
    }
}
